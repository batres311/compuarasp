...
class Features():
    ...
    def waveform(y,sr,res,archivo):
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(WAVEFORM_path_export1,WAVEFORM_path_export2,res,'waveform',fig, archivo)
        plt.close()
    ...
    def RootMeanSquaredError(y,res,archivo):
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(RMSE_path_export1,RMSE_path_export2,res,'RMSE',fig,archivo)
        plt.close()

        return t

    def ZeroCrossingRate(y,res,t,archivo):
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(ZCR_path_export1,ZCR_path_export2,res,'ZCR',fig,archivo)
        plt.close()

    def FreqAmp(y,sr, res,archivo):
    ...
    def MelSpectrogram(y,sr, res,archivo):
        
        #Extracting Mel Spectrogram n_fft=frame size
        mel_spectrogram = librosa.feature.melspectrogram(y=y, sr=sr, n_fft=N_FTT, hop_length=HOP_LENGTH, n_mels=90)
    ...
    def Chromagram(y,sr,res,archivo):
    ...
    def DeltaMFCCs(mfccs,sr,res,archivo):
    ...
    def Delta2MFCCs(mfccs,sr,res,archivo):
    ...
    def BandEnergyRatio(y,sr,res,archivo):
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(BER_path_export1,BER_path_export2,res,'Band Energy Ratio',fig,archivo)
        plt.close()

    def SpectralCentroid(y,sr,t,res,archivo):
        sc_y = librosa.feature.spectral_centroid(y=y, sr=sr, n_fft=N_FTT, hop_length=HOP_LENGTH)[0]
        #Visualising spectral centroid
        plt.figure(figsize=(25,10))
        fig, ax = plt.subplots()
        plt.plot(t, sc_y, color='b')
        ax.set(title="Spectral Centroid")
        ClaseAudio2.CargaeImagenAudio.guardarimagen(SpecCent_path_export1,SpecCent_path_export2,res,'Spectral Centroid',fig,archivo)
        plt.close()

    def Bandwidht(y,sr,t,res,archivo):
        
        #Spectral bandwidth with Librosa
        ban_y = librosa.feature.spectral_bandwidth(y=y, sr=sr, n_fft=N_FTT, hop_length=HOP_LENGTH)[0]
        #Visualising spectral bandwidth
        plt.figure(figsize=(25,10))
        fig, ax = plt.subplots()
        plt.plot(t, ban_y, color='b')
        ax.set(title="Bandwidth")
        ClaseAudio2.CargaeImagenAudio.guardarimagen(Bandwidth_path_export1,Bandwidth_path_export2,res,'Bandwidth',fig,archivo)
        plt.close()

    def SpectralContrast(y,sr,res,archivo):
        
        S = np.abs(librosa.stft(y,n_fft=N_FTT))#n_fft no iba
    ...
    def SpectralRollOff(y,S_db,sr,res,archivo):
        # Spectral Flatness
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(SpecRollOff_path_export1,SpecRollOff_path_export2,res,'Spectral Rolloff',fig,archivo)
        plt.close()
    ...
    def Tonnetz(y,sr,res,archivo):
        ...
        ClaseAudio2.CargaeImagenAudio.guardarimagen(Tonnetz_path_export1,Tonnetz_path_export2,res,'Tonnetz',fig,archivo)
        plt.close()
